...
public class ultimateCipher : MonoBehaviour {
    ...
    void Start ()
    {
        ...
        Debug.LogFormat("[Brown Cipher #{0}] Generated Word: {1}", moduleId, answer);     
        pages = new string[2][];
        pages[0] = new string[]{"", "", ""};
        pages[1] = new string[] { "", "", "" };
        string encrypt = browncipher(answer);
        pages[0][0] = encrypt.ToUpperInvariant();
        page = 0;
        getScreens();
    }
    string browncipher(string word)
    {
        string kw = wordList[2][UnityEngine.Random.Range(0, wordList[2].Count)].ToUpperInvariant();
        Debug.LogFormat("[Brown Cipher #{0}] Begin M-209 Encryption", moduleId);
        string encrypt = M209Enc(word.ToUpperInvariant(), kw.ToUpperInvariant());
        Debug.LogFormat("[Brown Cipher #{0}] Begin Strip Encryption", moduleId);
        encrypt = StripEnc(encrypt.ToUpperInvariant(), kw.ToUpperInvariant());
        Debug.LogFormat("[Brown Cipher #{0}] Begin Book Encryption", moduleId);
        BookEnc(kw.ToUpperInvariant());
        return encrypt;
    }
    string M209Enc(string word, string kw)
    {
        string[] binary =
        {
            "111000",
        ...
        for(int aa = 0; aa < 6; aa++)
        {
            ...
            Debug.LogFormat("[Brown Cipher #{0}] Rotor #{1}: {2}", moduleId, (aa + 1), rotors[aa]);
            Debug.LogFormat("[Brown Cipher #{0}] Number of Lugs triggering Rotor #{1}: {2}", moduleId, (aa + 1), lugs[aa]);
        }
        for (int aa = 0; aa < 6; aa++)
        {
            ...
            Debug.LogFormat("[Brown Cipher #{0}] {1} -> {2}", moduleId, word[aa], encrypt[aa]);
        }
        return encrypt;
    }
    string StripEnc(string word, string kw)
    {
        string key = getKey(kw, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", true);
        string[] strips = new string[6];
        for(int aa = 0; aa < 6; aa++)
        {
            int cursor = key.IndexOf(word[aa]);
            strips[aa] = key.Substring(cursor) + "" + key.Substring(0, cursor);
            Debug.LogFormat("[Brown Cipher #{0}] Key Row #{1}: {2}", moduleId, (aa + 1), strips[aa]);
        }
        int column = (Bomb.GetSerialNumberNumbers().Sum() % 25) + 1;
        Debug.LogFormat("[Brown Cipher #{0}] Column Chosen: {1}", moduleId, (column + 1));
        string encrypt = "";
        for(int aa = 0; aa < 6; aa++)
        {
    ...
    void BookEnc(string word)
    {
        ...
        for(int aa = 0; aa < word.Length; aa++)
        {
            ...
            Debug.LogFormat("[Blue Cipher #{0}] {1} -> {2}", moduleId, word[aa], temp);
            pages[0][1] = pages[0][1] + "" + temp[0];
            pages[0][2] = pages[0][2] + "" + temp[1];
            pages[1][0] = pages[1][0] + "" + temp[2];
            pages[1][1] = pages[1][1] + "" + temp[3];
            pages[1][2] = pages[1][2] + "" + temp[4];
        }
    }
